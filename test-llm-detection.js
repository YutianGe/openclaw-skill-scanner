...
import { analyzeFullSkillContent, MALICIOUS_BEHAVIOR_PATTERNS } from './src/ollama-analyzer.js';
...
async function runTests() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      const result = await analyzeFullSkillContent(testCase.code, testCase.name);

      console.log(`\n分析结果:`);
      console.log(`  判定: ${result.isSafe ? '✅ 安全' : '⚠️  恶意'}`);
      console.log(`  置信度: ${(result.confidence * 100).toFixed(1)}%`);

      if (result.threatScore !== undefined) {
        console.log(`  威胁评分: ${result.threatScore}/100 (${result.riskLevel})`);
      }

      console.log(`  可疑模式数: ${result.suspiciousPatterns?.length || 0}`);
      ...
      if (result.reason) {
        console.log(`\n  LLM 分析: ${result.reason.substring(0, 200)}`);
      }

      // 验证结果
      const isCorrect = (result.isSafe === !testCase.expectedMalicious);
      if (isCorrect) {
        console.log(`\n✅ 测试通过`);
        passed++;
      } else {
        console.log(`\n❌ 测试失败 - 预期: ${testCase.expectedMalicious ? '恶意' : '安全'}, 实际: ${result.isSafe ? '安全' : '恶意'}`);
        failed++;
      }
